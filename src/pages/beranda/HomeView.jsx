  ...
  return (
    <div className="min-h-screen bg-white p-6">
      <div className="text-center mb-10">
        <h1 className="text-4xl font-bold text-black-600 mb-4">Welcome to MyStream!</h1>
      ...
      <div className="mb-10">
        <h2 className="text-3xl font-semibold text-black-600 mb-4">Filter by Genre</h2>
        <div className="flex flex-wrap justify-center gap-2 mb-6">
          {genres.map((genre) => (
            <button
              ...
              className={`${
                selectedGenre === genre.id ? "bg-red-500" : "bg-red-200"
        ...
        {selectedGenre && (
          <div className="flex justify-center mt-4">
            <button
              ...
              className="bg-red-500 text-white hover:bg-red-600 transform hover:scale-105 transition-transform duration-200 ease-in-out rounded-md w-32 h-12"
            >
              Clear Genre
            </button>
          </div>
      ...
      {loading ? (
        ...
        <div>
          {genreDescription && (
            <p className="text-black text-center mb-4">{genreDescription}</p>
          )}
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {movies.length > 0 ? (
              movies.map((movie) => (
                <div
                  key={movie.id}
                  className="relative bg-black rounded-lg overflow-hidden shadow-lg transform hover:scale-105 transition-transform duration-300 cursor-pointer"
              ...
              <div className="flex justify-center items-center w-full h-64">
                <p className="text-black-600 text-center">No movies found for the selected genre.</p>
              </div>
            )}
          </div>
        </div>
